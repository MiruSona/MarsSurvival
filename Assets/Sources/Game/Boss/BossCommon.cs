...
public abstract class BossCommon : MonoBehaviour {
    ...
    //이펙트
    protected Renderer boss_effect = null;
    ...
    //공격 타이머
    protected GameDAO.Timer atk_timer = new GameDAO.Timer(0, 1f);
	...
	void FixedUpdate () {
        ...
        {
            if (ground_effect.gameObject.activeSelf)
            {
                if (ground_effect.isStopped)
                    ground_effect.gameObject.SetActive(false);
            }
        }
        #endregion
    ...
    #region 공통
    //자원 랜덤 생성
    protected void CreateSubstance(Define.Range bio, Define.Range metal, Define.Range crystal, int _crystal_drop_rate)
    {
        //0 ~ 9999 까지 랜덤 생성
        ...
        if (chance < _crystal_drop_rate)
            boss_data.give_subdata.crystal = Random.Range(crystal.min, crystal.max);
        else
