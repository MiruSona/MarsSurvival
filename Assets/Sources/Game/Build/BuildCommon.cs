...
public abstract class BuildCommon : MonoBehaviour {
    ...
    protected List<GameDAO.SPBossData> sp_boss_target = new List<GameDAO.SPBossData>();
    ...
    void OnTriggerEnter2D(Collider2D _col)
    {
        ...
        {
            sp_boss_target.Add(_col.GetComponent<SPBossCommon>().sp_boss_data);
            RecognizeEnter();
        }
    }
    ...
    void OnTriggerExit2D(Collider2D _col)
    {
        ...
        if (_col.CompareTag("SPBoss"))
        {
            sp_boss_target.Remove(_col.GetComponent<SPBossCommon>().sp_boss_data);
        }
    }
}
