...
public class UpgradeMenu : MonoBehaviour {
    ...
    private struct UpgradeUI
    {
        ...
        public Text repair_text;

        public UpgradeUI(Button _btn, Image _main_img, Image _sub_img, Text _level, Text _metal, Text _bio, Image _repair_need, Text _repair_text)
        {
            btn = _btn;
            main_img = _main_img; sub_img = _sub_img;
            level = _level; metal = _metal; bio = _bio;
            repair_need = _repair_need;
            repair_text = _repair_text;
        }
    }
    ...
    void Awake () {
        ...
        weapon_ui =
            new UpgradeUI(
                    ...
                    LoadTextComponent("Weapon", "UpgradeStop", "Text")
                );
        ...
        tool_ui =
            new UpgradeUI(
                    ...
                    LoadTextComponent("Tool", "UpgradeStop", "Text")
                );
        ...
        turret_ui =
            new UpgradeUI(
                    ...
                    LoadTextComponent("Turret", "UpgradeStop", "Text")
                );
        ...
        shield_ui =
            new UpgradeUI(
                    ...
                    LoadTextComponent("Shield", "UpgradeStop", "Text")
                );
    }
        ...
        if (player_data.my_subdata.CheckMore(weapon_data.upgrade_need[step, level]))
        {
            ...
            if (weapon_data.step == Define.weapon_step_max - 1 && weapon_data.level == Define.weapon_level_max - 1)
                if (Social.localUser.authenticated)
                    Social.ReportProgress(GPGSSstatics.achievement_power_overwhelming, 100.0f, (bool success) => {
                        if (success) {
                            Debug.Log("Reported");
                        } else {
                        }
    ...
    public void ToolUpgrade()
    {
        ...
        if (player_data.my_subdata.CheckMore(tool_data.upgrade_need[step]))
        {
            ...
            if (player_data.tool_data.step == Define.tool_step_max - 1)
                if (Social.localUser.authenticated)
                    Social.ReportProgress(GPGSSstatics.achievement_allpurposed, 100.0f, (bool success) => {
                        if (success) {
                            Debug.Log("Reported");
                        } else {
                        }
    ...
    private Text LoadTextComponent(string _name, string _sub_name1, string _sub_name2)
    {
        Text send_text = null;

        send_text = transform.FindChild(_name).FindChild(_sub_name1).FindChild(_sub_name2).GetComponent<Text>();
        ...
        if (weapon_data.level == 0)
        {
            ...
            if(spaceship_data.step < 1 && weapon_data.step == 1)
            {
                ...
                if (TextTrans.instance.languageCode == "KR") {
                    weapon_ui.repair_text.text = "2단계 우주선 수리가 필요합니다";
                } else if (TextTrans.instance.languageCode == "JP") {
                    weapon_ui.repair_text.text = "2段階までの宇宙船修理が必要です";
                } else {
                    weapon_ui.repair_text.text = "2nd Tier Spaceship repair is needed";
                }
            ...
            if (spaceship_data.step < 3 && weapon_data.step == 2)
            {
                ...
                weapon_ui.repair_need.gameObject.SetActive(true);
                if (TextTrans.instance.languageCode == "KR") {
                    weapon_ui.repair_text.text = "4단계 우주선 수리가 필요합니다";
                } else if (TextTrans.instance.languageCode == "JP") {
                    weapon_ui.repair_text.text = "4段階までの宇宙船修理が必要です";
                } else {
                    weapon_ui.repair_text.text = "4nd Tier Spaceship repair is needed";
                }
            }
        }
    }
    ...
    private void CheckToolStep()
    {
        ...
        if (spaceship_data.step < 1 && tool_data.step == 0)
        {
            ...
            if (TextTrans.instance.languageCode == "KR") {
                tool_ui.repair_text.text = "2단계 우주선 수리가 필요합니다";
            } else if (TextTrans.instance.languageCode == "JP") {
                tool_ui.repair_text.text = "2段階までの宇宙船修理が必要です";
            } else {
                tool_ui.repair_text.text = "2nd Tier Spaceship repair is needed";
            }
        }
        ...
        if (spaceship_data.step < 3 && tool_data.step == 1)
        {
            ...
            tool_ui.repair_need.gameObject.SetActive(true);
            if (TextTrans.instance.languageCode == "KR") {
                tool_ui.repair_text.text = "4단계 우주선 수리가 필요합니다";
            } else if (TextTrans.instance.languageCode == "JP") {
                tool_ui.repair_text.text = "4段階までの宇宙船修理が必要です";
            } else {
                tool_ui.repair_text.text = "4nd Tier Spaceship repair is needed";
            }
        }
    }
    ...
    private void CheckBuildStep(UpgradeUI _ui, GameDAO.BuildData _build_data)
    {
        ...
        if (spaceship_data.step < 1 && _build_data.step == 0)
        {
            ...
            if (TextTrans.instance.languageCode == "KR") {
                _ui.repair_text.text = "2단계 우주선 수리가 필요합니다";
            } else if (TextTrans.instance.languageCode == "JP") {
                _ui.repair_text.text = "2段階までの宇宙船修理が必要です";
            } else {
                _ui.repair_text.text = "2nd Tier Spaceship repair is needed";
            }
        }
        ...
        if (spaceship_data.step < 3 && _build_data.step == 1)
        {
            ...
            _ui.repair_need.gameObject.SetActive(true);
            if (TextTrans.instance.languageCode == "KR") {
                _ui.repair_text.text = "4단계 우주선 수리가 필요합니다";
            } else if (TextTrans.instance.languageCode == "JP") {
                _ui.repair_text.text = "4段階までの宇宙船修理が必要です";
            } else {
                _ui.repair_text.text = "4nd Tier Spaceship repair is needed";
            }
        }
    }
    #endregion
