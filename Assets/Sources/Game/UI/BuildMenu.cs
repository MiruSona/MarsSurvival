...
public class BuildMenu : MonoBehaviour {
    ...
    private GameDAO.ArmorData armor_data;

    private Animator player_anim;

    public GameObject armorPop;
    public GameObject armor1Btn;
    public GameObject armor2Btn;

    //빌드 UI
    private struct BuildUI {
        public Button btn;
        public Image img;
        public Text level, metal, bio, crystal;
        public Image limit;

        public BuildUI(Button _btn, Image _img, Text _level, Text _metal, Text _bio, Text _crystal, Image _limit) {
            btn = _btn; img = _img;
            level = _level;
            metal = _metal; bio = _bio; crystal = _crystal;
            limit = _limit;
        }
    }
    ...
    void Awake() {
        ...
        armor_data = GameDAO.instance.armor_data;
        ...
        mine_ui =
            new BuildUI(
                ...
                null,
                null,
                LoadTextComponent("Mine", "CrystalText"),
    ...
    public void ChooseWarp() {
        ...
        if (player_data.my_subdata.CheckMore(warp_data.buy_need)) {
            ...
            if (Social.localUser.authenticated)
                Social.ReportProgress(GPGSSstatics.achievement_power_of_crystal, 100.0f, (bool success) => {
                    if (success) {
                        Debug.Log("Reported");
                    } else {
                    }
    ...
    public void ChooseArmor() {
        ...
        if (SystemDAO.instance.file_manager.cloth_buy == Define.NORMAL_CLOTH) {
            armor1Btn.GetComponent<Button>().interactable = false;
        } else if (SystemDAO.instance.file_manager.cloth_buy == Define.SPECIAL_CLOTH) {
            armor1Btn.GetComponent<Button>().interactable = false;
            armor2Btn.GetComponent<Button>().interactable = false;
        }
        if(!player_data.my_subdata.CheckMore(armor_data.buy_need)) {
            armor1Btn.GetComponent<Button>().interactable = false;
        } else {
            armor1Btn.GetComponent<Button>().interactable = true;
        }
    }

    public void ArmorPopOff() {
        armorPop.SetActive(false);
    }

    public void Armor1() {
        if (player_data.my_subdata.CheckMore(armor_data.buy_need)){
            SystemDAO.instance.file_manager.SaveClothBuy(Define.NORMAL_CLOTH);
            player_data.my_subdata.SubAll(armor_data.buy_need);
            armorPop.SetActive(false);
        }
    }
    public void Armor2() {
        MarketService.instance.BuyProductID("armor");
        armorPop.SetActive(false);
    }
    ...
    private void SetBuildUI(BuildUI _build_ui, GameDAO.BuildData _build_data, bool _empty) {
        ...
        if (_build_ui.crystal != null && need_sub.crystal != 0)
        {
            _build_ui.crystal.text = "x" + need_sub.crystal;
            ChangeText(_build_ui.crystal, 0, 0, need_sub.crystal);
        }
