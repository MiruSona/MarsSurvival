...
public class GameDAO : SingleTon<GameDAO> {
    ...
    public class PlayerData
    {
        ...
        //중독 타이머
        public Timer poisoned_timer = new Timer(0f, 120.0f);
        ...
        {
            ArtifactData artifact_data = instance.artifact_data;
            //만약 아티펙트 작동중이면 이속 * 1.2
            if (artifact_data.artifcat[1].activate)
            {
                speed *= artifact_data.artifcat[1].value;
                max_speed *= artifact_data.artifcat[1].value;
            }
            //만약 아티펙트 작동중이면 체력 * 1.2
            if (artifact_data.artifcat[2].activate)
            {
                hp *= artifact_data.artifcat[2].value;
                max_hp *= artifact_data.artifcat[2].value;
            }

            //무기랑 도구도 체크
            weapon_data.CheckArtifact();
            tool_data.CheckArtifact();
        }
    ...
    public class ToolData
    {
        ...
        {
            ArtifactData artifact_data = instance.artifact_data;
            //만약 아티펙트 작동중이면 도구 속도 * 1.1
            if (artifact_data.artifcat[3].activate)
            {
                for(int i = 0; i < speed.Length; i++)
                    speed[i] *= artifact_data.artifcat[3].value;
            }
        }
    }
    ...
    public class WeaponData
    {
        ...
        {
            ArtifactData artifact_data = instance.artifact_data;
            //만약 아티펙트 작동중이면 공격력 * 1.5
            if (artifact_data.artifcat[0].activate)
            {
                for (int i = 0; i < Define.weapon_step_max; i++)
                    for (int j = 0; j < Define.weapon_level_max; j++)
                        atk[i, j] *= artifact_data.artifcat[0].value;
            }
        }
    }
    #endregion
    ...
    public class TurretData : BuildData
    {
        ...
        public TurretData()
        {
            ...
            {
                new SubstanceData(60,0,0),
                new SubstanceData(80,0,0),
                new SubstanceData(108,0,0)
            };
    ...
    public class ShieldData : BuildData
    {
        ...
        public ShieldData()
        {
            ...
            build_need = new SubstanceData[Define.build_step_max]
            {
                new SubstanceData(45,60,0),
                new SubstanceData(72,96,0),
                new SubstanceData(108,144,0)
            };
    ...
    public class MealData : ConsumeData
    {
        ...
        {
            ArtifactData artifact_data = instance.artifact_data;
            //만약 아티펙트 작동중이면 회복력 + 500
            if (artifact_data.artifcat[4].activate)
            {
                recovery += artifact_data.artifcat[4].value;
            }
        }
    }
    ...
    public class ArtifactData
    {
        ...
        public Artifact[] artifcat =
            new Artifact[Define.artifact_num]
            {
                ...
                new Artifact("Fragment", 1.1f),     //도구 속도 10%
        ...
        public ArtifactData()
        {
            artifcat[0].description = "Increase Attack Point 50%";
            artifcat[1].description = "Increase Move Speed 20%";
            artifcat[2].description = "Increase Health Point 20%";
            artifcat[3].description = "Increase Tool Speed 10%";
            artifcat[4].description = "Increase Meal Point +500";
        }
