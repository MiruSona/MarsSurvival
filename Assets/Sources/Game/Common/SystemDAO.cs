...
public class SystemDAO : SingleTon<SystemDAO> {
    ...
    public class MapManager {
        ...
        #region 특수 보스
        //보스 실제 위치 반환
        public Vector3 GetSPBossRealPoint(float _offset_y)
        {
            return ConvertToRealPointF(sp_boss_point, _offset_y);
        }

        //보스 가상 위치 설정
        public void SetSPBossPoint(Transform _transform)
        {
            sp_boss_point = ConvertToVirtualPointF(_transform);
        }

        //보스 랜덤 가상 위치 추가
        public void RandomSPBossPoint()
        {
            float random = 0;
            ...
            if (player_point < 0)
            {
                min = Define.sp_boss_left_range.min;
                max = Define.sp_boss_left_range.max;
            }
            else
            {
                min = Define.sp_boss_right_range.min;
                max = Define.sp_boss_right_range.max;
            }

            random = UnityEngine.Random.Range(min, max);
            sp_boss_point = random;
        }

        //주인공 근처 위치 구하기
        public void RandomAreaSPBossPoint()
        {
            float point = 0;
            int random = UnityEngine.Random.Range(0, 2);

            if(player_point - Define.sp_boss_area < Define.init_vpos_minus)
            {
                point = player_point + Define.sp_boss_area;
            }
            else if(player_point + Define.sp_boss_area > Define.init_vpos_plus)
            {
                point = player_point - Define.sp_boss_area;
            }
            else
        ...
        //파괴된 보스 가상 위치 설정
        public void DestroySPBossPoint()
        {
            //오른쪽 끝 좌표로
            sp_boss_point = Define.init_vpos_plus;
        }
        #endregion
    ...
    public class FileManager {
        ...
        //보석 주는 창 띄웠는지(5일째) 여부
        public bool give_crystal = false;

        //클리어 횟수
        public int clear_num = 0;

        //하드모드 여부
        public bool hard_mod = false;

        //광고 제거 구매 여부
        public bool commercial_off = false;

        //갑옷 구매 여부
        public int cloth_buy = Define.NO_CLOTH;
        ...
        {
            public int buff_kind = 0;
            public float buff_timer = 0;
            public bool enable = false;
            public bool already_clicked = false;
        ...
        public class SPBossData
        {
            ...
            public GameDAO.SPBossData data = null;
        ...
        public SPBossData sp_boss_data = new SPBossData();
        ...
        #region 독립 데이터 처리
        //하드모드 기록
        public void SaveHardMod()
        {
            SaveFile(Define.hard_mod_key);
        }
        ...
        //광고 제거 여부 세이브
        public void SaveCommerciealOff()
        {
            SaveFile(Define.commercial_off_key);
        }

        //광고 제거 여부 로드
        public void LoadCommerciealOff()
        {
            LoadFile(Define.commercial_off_key);
        }

        //갑옷 구매 여부 세이브
        public void SaveClothBuy(int _cloth_value)
        {
            cloth_buy = _cloth_value;
            SaveFile(Define.cloth_buy_key);
        }

        //갑옷 구매 여부 로드
        public void LoadClothBuy()
        {
            LoadFile(Define.cloth_buy_key);
        }

        //언어 코드 세이브
        public void SaveLanguageCode(string _code) {
            languageCode = _code;
            SaveFile(Define.language_code_key);
        }

        //언어 코드 로드
        public void LoadLanguageCode() {
            LoadFile(Define.language_code_key);
        }
        ...
        //보석 주는 메뉴 bool 세이브
        public void SaveGiveCrystal()
        {
            SaveFile(Define.give_crystal_key);
        }
        #endregion
        ...
        public void RecordDatas()
        {
            ...
            GameDAO.BuffData bfd = GameDAO.instance.buff_data;
            ...
            #region 버프 데이터
            buff_data.buff_kind = bfd.buff_kind;
            buff_data.buff_timer = bfd.buff_timer.timer;
            buff_data.enable = bfd.enable;
            buff_data.already_clicked = bfd.already_clicked;
            #endregion
            ...
            #region 특수 보스 데이터
            sp_boss_data.regen_timer = gm.sp_boss_regen_timer.timer;
            sp_boss_data.sp_boss_step = gm.sp_boss_step;
            if (gm.sp_boss_spawn == null)
                sp_boss_data.data = null;
            else
                sp_boss_data.data = gm.sp_boss_spawn.GetComponent<SPBossCommon>().sp_boss_data;

            sp_boss_data.point = mm.sp_boss_point;
            #endregion
        ...
        public void LoadDatas()
        {
            ...
            #region 버프 데이터
            bfd.buff_kind = buff_data.buff_kind;
            bfd.buff_timer.timer = buff_data.buff_timer;
            bfd.enable = buff_data.enable;
            bfd.already_clicked = buff_data.already_clicked;
            #endregion
            ...
            #region 특수 보스 데이터
            gm.sp_boss_regen_timer.timer = sp_boss_data.regen_timer;
            gm.sp_boss_step = sp_boss_data.sp_boss_step;
            mm.sp_boss_point = sp_boss_data.point;
            #endregion
        ...
        public void SaveAll() {
            ...
            SaveFile(Define.sp_boss_data_key);
        ...
        public void LoadAll() {
            ...
            LoadFile(Define.give_crystal_key);
            ...
            LoadFile(Define.sp_boss_data_key);
        ...
        public void DeleteBase() {
            ...
            PlayerPrefs.DeleteKey(Define.buff_data_key);
            ...
            PlayerPrefs.DeleteKey(Define.sp_boss_data_key);
        ...
        public void SaveFile(string key) {
            ...
            switch (key) {
                case Define.hard_mod_key:
                    binary_formatter.Serialize(memory_stream, hard_mod);
                    break;
                ...
                case Define.commercial_off_key:
                    binary_formatter.Serialize(memory_stream, commercial_off);
                    break;

                case Define.cloth_buy_key:
                    binary_formatter.Serialize(memory_stream, cloth_buy);
                    break;

                case Define.language_code_key:
                    binary_formatter.Serialize(memory_stream, languageCode);
                    break;
                ...
                case Define.give_crystal_key:
                    binary_formatter.Serialize(memory_stream, give_crystal);
                    break;
                ...
                case Define.buff_data_key:
                    binary_formatter.Serialize(memory_stream, buff_data);
                    break;
                ...
                case Define.sp_boss_data_key:
                    binary_formatter.Serialize(memory_stream, sp_boss_data);
                    break;
        ...
        public void LoadFile(string key) {
            ...
            if (!string.IsNullOrEmpty(data)) {
                ...
                switch (key) {
                    case Define.hard_mod_key:
                        hard_mod = (bool)binary_formatter.Deserialize(memory_stream);
                        break;
                    ...
                    case Define.commercial_off_key:
                        commercial_off = (bool)binary_formatter.Deserialize(memory_stream);
                        break;

                    case Define.cloth_buy_key:
                        cloth_buy = (int)binary_formatter.Deserialize(memory_stream);
                        break;

                    case Define.language_code_key:
                        languageCode = (string)binary_formatter.Deserialize(memory_stream);
                        break;
                    ...
                    case Define.give_crystal_key:
                        give_crystal = (bool)binary_formatter.Deserialize(memory_stream);
                        break;
                    ...
                    case Define.buff_data_key:
                        buff_data = (BuffData)binary_formatter.Deserialize(memory_stream);
                        break;
                    ...
                    case Define.sp_boss_data_key:
                        sp_boss_data = (SPBossData)binary_formatter.Deserialize(memory_stream);
                        break;
