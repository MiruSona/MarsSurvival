...
public abstract class MonsterCommon : MonoBehaviour {
    ...
    //공격 타이머
    protected GameDAO.Timer atk_melee_timer = new GameDAO.Timer(0, 0.1f);

    //플레이어와 몬스터 사이 거리
    protected Vector3 between_distance;

    //스프라이트 높이
    protected float sprite_height = 0;
    ...
    void FixedUpdate () {
        ...
        //몬스터 가상 위치 갱신
        map_manager.SetMonsterPoint(transform, monster_data.index);
        //몬스터가 diable point 넘어가면 쥬금!
        if (map_manager.CheckDisableMonster(monster_data.index))
        {
            DieWithDisable();
            return;
        }
        ...
        switch (monster_data.movement)
        {
            ...
            case GameDAO.MonsterData.Movement.isAttackMelee:
                ...
                //공격
                if (atk_melee_timer.timer < atk_melee_timer.term)
                    atk_melee_timer.timer += Time.deltaTime;
                else
                {
                    //플레이어 공격중이면
                    if (attack_player)
    ...
    #region 공통
    //자원 랜덤 생성
    protected void CreateSubstance(Define.Range bio, Define.Range metal, int _bio_drop_rate, int _metal_drop_rate, int _crystal_drop_rate)
    {
        //0 ~ 9999 까지 랜덤 생성
        ...
        if (chance < _crystal_drop_rate)
            monster_data.give_subdata.crystal = 1;
        else
    ...
    protected void Die()
    {
        ...
        //플레이어 크리스탈 획득 이펙트
        if (monster_data.give_subdata.crystal > 0 && !get_crystal_ui.activeSelf)
        ...
        //가상 위치 제거
        map_manager.DestroyMonsterPoint(monster_data.index);
        //게임메니저에서 몬스터 제거
        game_manager.DestroyMonster(monster_data.index, monster_data.pool);
    ...
    public void Regen()
    {
        ...
        monster_data.state = GameDAO.MonsterData.State.isAlive;
        //랜덤 위치 추가
        map_manager.AddRandomMonsterPoint(monster_data.index, global_state.level);
        //위치 초기화
        transform.localPosition = map_manager.GetMonsterRealPoint(0, monster_data.index);
        //중력 초기화
        rb2d.gravityScale = 1f;
        //레이어 초기화
        sprite_renderer.sortingOrder = monster_data.index;
        if (monster_effect != null)
            monster_effect.sortingOrder = monster_data.index - 1;
        //엑티브 true
        gameObject.SetActive(true);
    }

    //로드
    public void Load(GameDAO.MonsterData _monster_data)
    {
        //엑티브 true
        gameObject.SetActive(true);
        //데이터 초기화
        monster_data = _monster_data;
        //움직임 초기화
        monster_data.movement = GameDAO.MonsterData.Movement.isMove;
        //위치 초기화
        if (monster_data.pool == GameDAO.MonsterData.Pool.SandMonster_Weak || monster_data.pool == GameDAO.MonsterData.Pool.SandMonster_Normal)
            sprite_height = 0;
        transform.localPosition = map_manager.GetMonsterRealPoint(sprite_height, monster_data.index);
        //중력 초기화
        rb2d.gravityScale = 1f;
        //레이어 초기화
        sprite_renderer.sortingOrder = monster_data.index;
