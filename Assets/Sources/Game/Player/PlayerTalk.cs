...
public class PlayerTalk : MonoBehaviour {
    ...
    //타이머
    private GameDAO.Timer talk_timer = new GameDAO.Timer(0f, Define.Day_Cycle / 5.0f);
    private GameDAO.Timer stay_timer = new GameDAO.Timer(0f, 3.5f);
    ...
    //섹터 오픈 시 하는 대사
    private string[] open_sentences_kr = new string[2]
    {
        "모험 영역이 확장되었습니다.",
        "모험 영역이 확장되었습니다."
    };
    private string[] open_sentences_jp = new string[2]
{
        "冒険の領域が広がりました。",
        "冒険の領域が広がりました。"
};
    private string[] open_sentences_en = new string[2]
{
        "Your adventure area was expanded.",
        "Your adventure area was expanded."
};
    ...
    void Start() {
        //대사 가져오기
        TextAsset txt_file = Resources.Load<TextAsset>("Etc/PlayerSentences_" + TextTrans.instance.languageCode);
        sentences = txt_file.text.Split('\n');
        txt_file = Resources.Load<TextAsset>("Etc/Tips_" + TextTrans.instance.languageCode);
        ...
        switch (_talk_kind)
        {
            ...
            case TalkKind.OpenSector:
                ...
                if (TextTrans.instance.languageCode == "KR") {
                    player_talk.text = open_sentences_kr[global_state.level - 1];
                } else if (TextTrans.instance.languageCode == "JP") {
                    player_talk.text = open_sentences_jp[global_state.level - 1];
                } else {
                    player_talk.text = open_sentences_en[global_state.level - 1];
                }
                break;
