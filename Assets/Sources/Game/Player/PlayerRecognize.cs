...
public class PlayerRecognize : MonoBehaviour {
    ...
    void OnTriggerStay2D(Collider2D _col) {
        ...
        {
            if (player_data.target_bigsub == null)
            {
                //자원 채취로 상태 변경
        ...
        //큰 자원인지 확인
        if (_col.CompareTag("BigSubstance"))
        {
            if (player_data.target_substance == null)
            {
                ...
                player_data.movement = GameDAO.PlayerData.Movement.isGather;
                //타겟 자원 설정
                player_data.target_bigsub = _col.GetComponent<BigSubstance>();
                //타겟 위치 설정
                Vector3 pos = _col.transform.position;
                pos.y -= 1.5f;
                player_data.tool_data.target_position = pos;
            }
        }
