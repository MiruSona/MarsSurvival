...
public class PlayerBullet : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        //보스에게 닿을 시
        if (col.CompareTag("SPBoss"))
        {
            GameDAO.SPBossData sp_boss_data = null;
            sp_boss_data = col.GetComponent<SPBossCommon>().sp_boss_data;
            //보스가 살아있는 상태면 데미지!
            if (sp_boss_data.state == GameDAO.SPBossData.State.isAlive)
                sp_boss_data.SubHP(weapon_data.atk[weapon_data.step, weapon_data.level]);
            //장전
            Reload();
        }
