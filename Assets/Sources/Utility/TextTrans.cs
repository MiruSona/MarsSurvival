...
public class TextTrans : SingleTon<TextTrans> {
    ...
    void Awake() { //OnLevelWasLoaded보다 Start가 느림...
        ...
        languageCode = file_manager.languageCode;
        Debug.Log(languageCode);

        if (file_manager.languageCode == null) { //세이브 없을 때 시작시 언어 변경함
            if (Application.systemLanguage == SystemLanguage.Korean) { //한글
                languageCode = "KR";
                file_manager.SaveLanguageCode("KR");
                fontset = Resources.Load<Font>("Etc/Font/DungGeunMo");
            } else if (Application.systemLanguage == SystemLanguage.Japanese) { //일본어
                languageCode = "JP";
                file_manager.SaveLanguageCode("JP");
                fontset = Resources.Load<Font>("Etc/Font/03スマートフォントUI");
            } else { //그 외엔 전부 영어로 처리함
                languageCode = "EN";
